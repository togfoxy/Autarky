...
local function DrawZone(zne,r,g,b,alpha,caption)
	...
	love.graphics.print(caption, zne.x+2, zne.y + 50)
	
	local imgIndex
	if caption == "Healer" then imgIndex = 2 end
	if caption == "Fruit shop" then imgIndex = 3 end
	if caption == "Lumber" then imgIndex = 4 end
	if caption == "Cotton" then imgIndex = 5 end
	if caption == "Cloth" then imgIndex = 6 end
	
	local myx = zne.x + (gintZoneSize / 2) - (garrImage[imgIndex]:getWidth() / 2)
	love.graphics.setColor(1,1,1,1)
	love.graphics.draw(garrImage[imgIndex],myx,zne.y)		-- drawable, x, y, rotation, scale x, scale y
...
function drawobjects.DrawWorld()
	...
	-- background
	local myscale = gintScreenWidth / 451
	...
	for k,v in ipairs(Zones) do
		...
		if v.zonetype == enum.zonetypeWater then
			...
			local myx = v.x + (gintZoneSize / 2) - (garrImage[7]:getWidth() / 2)
			love.graphics.setColor(1,1,1,1)
			love.graphics.draw(garrImage[7],myx,v.y)		-- drawable, x, y, rotation, scale x, scale y			
			
			
		end
		if v.zonetype == enum.zonetypeLumberyard then
			DrawZone(v,208/255,156/255,52/255,1,"Lumber")
		end
		...
		if v.zonetype == enum.zonetypeHeal then
			DrawZone(v,218/255,73/255,73/255,1,"Healer")
		end	
	...
	if gbolPaused then
		love.graphics.setColor(1,1,1,1)
		love.graphics.print("Paused", 900, 550)
	end
end
...
-- agt = Agents
	...
	for k, v in ipairs(agt) do
		...
		love.graphics.print(tmpstr, x+13,y - 8)
